...
public class Node {
    ...
    /**
     * Returns the neareast <code>Entry</code> to the given
     * <code>Cluster</code>. The distance is minimized over
     * <code>Entry.calcDistance(Cluster)</code>.
     *
     ...
     *
     * @param newEntry The entry to which the entry with the minimal distance to
     * it is calculated.
     ...
     * @throws NoFreeEntryException Is thrown when there is no space left in the
     ...
     * @throws NoFreeEntryException Is thrown when there is no free entry left
     * in the node.
     */
    ...
    /**
     * If there exists an entry, whose relevance is under the threshold given as
     * a parameter to the tree, this entry is returned. Otherwise
     ...
     * the <code>entries</code> array so that the non-empty entries are still at
     * the beginning.
     *
