...
/**
 * Citation: ClusTree: Philipp Kranen, Ira Assent, Corinna Baldauf, Thomas
 * Seidl: The ClusTree: indexing micro-clusters for anytime stream mining.
...
public class ClusTree extends AbstractClusterer {
    ...
    /**
     * insert newPoint into the tree using the BreadthFirst strategy, i.e.:
     * insert into the closest entry in a leaf node.
     *
     * @param newPoint
     * @param budget
     * @param timestamp
    ...
    /**
     * This method checks if there is an outdated (or empty) entry on the path
     * from node to root. It updates the weights of nodes on path and then
     * checks if it is outdated.
     *
     * @param node
    ...
    /**
     * recalculates data for all entries, that lie on the path from the root to
     * the Entry toUpdate.
     */
    ...
    private Entry insertHere(Entry newEntry, Node currentNode,
            Entry parentEntry, ClusKernel carriedBuffer, Budget budget, long timestamp) {
    ...
    /**
     * This method calculates the distances between the new point and each Entry
     * in a leaf node. It returns the node that contains the entry with the
    ...
    /**
     * Inserts an <code>Entry</code> into a <code>Node</code> without inducing a
     * split.
     *
     ...
     * @param parentEntry The entry in the tree that points at the node that is
...
//                    }
    ...
    class BestMergeInNode {
        ...
        /**
         * The constructor of this return value. It will automatically make sure
         * that the first position is the smaller one of the two.
         *
